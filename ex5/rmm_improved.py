...
def rank_maximal_matching(G, rank="rank", top_nodes=None):
    ...
    for i in range(min_rank, max_rank):
        ...
        #remove_edges_ver3(graph, odd, unreachable, i, rank=rank)
        remove_edges_ver3_process(graph, odd, unreachable, i, rank=rank)
        rm_OiUi(graph, odd, unreachable)
        rm_OiOi(graph, odd)
...
def get_max_and_min_rank(G, rank="rank"):
    logging.info('find the minimum and the maximum rank in G')
    x = {d[rank] for (u, v, d) in G.edges(data=True)}
    return max(x), min(x)

@my_timer
def divide_to_sets(Gi, matched_edges, free_nodes):
    logging.info('divide Gi vertices to even, odd and unreachable sets')
    """
...
def find_free_vertices(Gi: nx.Graph, M):
    logging.info('find free vertices in Gi')
    """
    Gi - is a graph with i' ranked edges
...
def create_Gi(G, Gi, rank_i, rank="rank"):
    logging.info('create Gi')
    Gi.add_edges_from(
        [(u, v, d) for (u, v, d) in G.edges(data=True) if d[rank] == rank_i]
    )


"""def create_small_graph(s,G,rank_i,rank="rank"):
    ranks = {}
    edges = []
    for (u, v, d) in G.edges(data=True):
        if u in s or v in s:
            if d[rank] > rank_i:  # remove rank> rank_i
                continue
        edges.append((u, v))
        ranks[(u,v)] = {rank: d[rank]}
    return edges,ranks"""


def delete_edges(s,G,rank_i,rank="rank"):
    edges = set()
    for (u, v, d) in G.edges(data=True):
        if u in s or v in s:
            if d[rank] > rank_i:  # remove rank> rank_i
...
@my_timer
def remove_edges_ver3(G, Oi, Ui, rank_i, rank="rank"):

    edges_Oi = delete_edges(Oi, G, rank_i,rank=rank)
    edges_Ui = delete_edges( Ui, G, rank_i,rank=rank)
    edges = list(edges_Ui.union(edges_Oi))
    G.remove_edges_from(edges)

@my_timer
def remove_edges_ver3_process(G, Oi, Ui, rank_i, rank="rank"):

    with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
        p1 = executor.submit(delete_edges, Oi, G, rank_i,rank=rank)
        p2 = executor.submit(delete_edges, Ui, G, rank_i, rank=rank)
        edges = list(p1.result().union(p2.result()))
    G.remove_edges_from(edges)

"""
@my_timer
def remove_edges_ver2(G, Oi, Ui, rank_i, rank="rank"):
    G_tmp = nx.Graph()
    edges_Oi,ranks_Oi = create_small_graph(Oi, G, rank_i,rank=rank)
    edges_Ui,ranks_Ui = create_small_graph( Ui, G, rank_i,rank=rank)
    G_tmp.add_edges_from(edges_Oi)
    G_tmp.add_edges_from(edges_Ui)
    nx.set_edge_attributes(G_tmp,ranks_Oi)
    nx.set_edge_attributes(G_tmp,ranks_Ui)
    return G_tmp
"""

"""
@my_timer
def remove_edges_ver2_process(G, Oi, Ui, rank_i, rank="rank"):

    G_tmp = nx.Graph()
    G_tmp.nodes = copy.copy(G.nodes)
    with concurrent.futures.ProcessPoolExecutor(max_workers=2) as executor:
        futures = [executor.submit(create_small_graph, s, G, rank_i,rank=rank) for s in [Oi,Ui]]
        for future in concurrent.futures.as_completed(futures):  # return each result as soon as it is completed:
            G_tmp.add_edges_from(future.result())
    ...
    for i in range(SIZE):
        ...
        for i in range(SIZE):
            ...
            if random.random() > 0.5:
                ...
                G.add_edge(src, dst, weight=rank)

    M = rank_maximal_matching(G, rank="weight", top_nodes=left)
    print(M)